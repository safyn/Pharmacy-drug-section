...
$sql = "SELECT DrugID, Brand, Name, form, Strength, usageInstructions, SupplierName, Price, RetailPrice, reorderLevel, ReorderQuantity,sideEffects from Drug where exist =1";
...
echo "<select name = 'listbox' id='listbox' onclick = 'populate()' >"; // Display select tag
...
while($row = mysqli_fetch_array($result)){
	...
	$instructions = $row['usageInstructions'];
 ...
 // assign all values into one variable that can be split with "," later on 
    $alltext = "$id,$brandName,$genericName,$form,$strength,$instructions,$supplier
                ,$costPrice,$retailPrice,$reorderLevel,$reorderQuantity,$sideEffects";
